...
impl Host {
    pub fn get_hosts_by_project_id(conn: &mut PgConnection, id_project: Uuid) -> QueryResult<Vec<Host>> {
    ...
    pub fn create_host(conn: &mut PgConnection, form: &HostForm, id_project: Uuid) -> QueryResult<Host> {
    ...
    pub fn delete_host(conn: &mut PgConnection, host_id: i32) -> QueryResult<usize> {
        ...
        conn.transaction(|conn| {
            diesel::delete(hosts.filter(id.eq(host_id)))
                .execute(conn)
        })
    }
    
    pub fn update_host(conn: &mut PgConnection, form: &HostForm, id_project: Uuid, host_id: i32) -> QueryResult<usize> {
        use crate::db::schema::hosts::dsl::*;
        let new_host = NewHost {
            hostname: form.hostname.clone(),
            ip_address: form.ip_address.clone(),
